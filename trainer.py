...
from torch.utils.data import DataLoader, RandomSampler, WeightedRandomSampler, SequentialSampler, ConcatDataset
from transformers import AdamW, get_linear_schedule_with_warmup
import copy

from TorchDataset import ConcatTorchDataset, TorchDataset
from utils import set_seed, write_f1_tc, write_prediction_re, write_prediction_tc, write_prediction_wic, \
    compute_metrics, MODEL_CLASSES, WiCMODEL_CLASSES, ReMODEL_CLASSES, ContrastiveLoss

logger = logging.getLogger(__name__)

# number of data in each class
class_data = {
    "agnews": [(0, 30000), (1, 21000), (2, 12000), (3, 3000)]
}
...
class Trainer(object):
    def __init__(self, args, train_dataset=None, dev_dataset=None, test_dataset=None, labelset=None, unlabeled=None,
        ...
        self.concat_dataset = train_dataset
    ...
    # re-sampling weight
    def get_resampling_weight(self, label):
        max_class = max(class_data[self.args.task], key=lambda x: x[1])
        sorted_class = sorted(class_data[self.args.task], key=lambda x: x[1], reverse=True)
        num_classes = len(class_data[self.args.task])
        for l in range(len(sorted_class)):
            if sorted_class[l][0] == label:
                return (sorted_class[num_classes - l - 1][1] / max_class[1]) ** self.args.sampling_rate
        print(label)
        # selected_list = np.random.choice(self.args.num_classes, len(batch[0]), p=Mu)
    ...
    def set_dataset(self):
        ...
        train_sampler = RandomSampler(self.concat_dataset)
        train_dataloader = DataLoader(self.concat_dataset, sampler=train_sampler, batch_size=self.args.batch_size)
        # assert 0
    ...
    def self_train(self, soft=True):

        # generating pseudo_labels
        self_train_dataset = ConcatTorchDataset([self.train_dataset, self.unlabeled])
        # self_train_dataset = ConcatDataset([self.train_dataset, self.unlabeled])

        train_sampler = RandomSampler(self_train_dataset)

        train_dataloader = DataLoader(self_train_dataset, sampler=train_sampler, batch_size=self.args.batch_size)
        if self.args.self_training_max_step > 0:
            t_total = self.args.self_training_max_step
            self.args.num_train_epochs = self.args.self_training_max_step // (
                    len(train_dataloader) // self.args.gradient_accumulation_steps) + 1
        else:
            t_total = len(train_dataloader) // self.args.gradient_accumulation_steps * self.args.num_train_epochs
        ...
        for t3 in range(int(self_training_steps / len(train_dataloader)) + 1):
            # update pseudo_labels
            indices = torch.IntTensor().to(self.device)
            batch_output = torch.Tensor().to(self.device)
            batch_label = torch.IntTensor().to(self.device)
            self.concat_dataset = ConcatTorchDataset([self.train_dataset])
            ...
            for step, batch in enumerate(epoch_iterator):
                if global_step % self.args.self_training_update_period == 0:
                    ...
                    if global_step != 0:
                        ...
                        label0_discard_indices = list(torch.utils.data.RandomSampler(
                            (unlabeled_batch_output == 0).nonzero(), num_samples=int(
                                len((unlabeled_batch_output == 0).nonzero()) * (1 - self.get_resampling_weight(0)))
                            , replacement=True))
                        label1_discard_indices = list(torch.utils.data.RandomSampler(
                            (unlabeled_batch_output == 1).nonzero(), num_samples=int(
                                len((unlabeled_batch_output == 1).nonzero()) * (1 - self.get_resampling_weight(1)))
                            , replacement=True))
                        label2_discard_indices = list(torch.utils.data.RandomSampler(
                            (unlabeled_batch_output == 2).nonzero(), num_samples=int(
                                len((unlabeled_batch_output == 2).nonzero()) * (1 - self.get_resampling_weight(2)))
                            , replacement=True))
                        unlabeled_batch_output[0][label0_discard_indices] = -1
                        unlabeled_batch_output[0][label1_discard_indices] = -1
                        unlabeled_batch_output[0][label2_discard_indices] = -1

                        self_train_dataset.update_label(unlabeled_indices, unlabeled_batch_output, 3)
                        updated_indices = unlabeled_indices[0][torch.reshape((unlabeled_batch_output[0] != -1).nonzero()
                                                                             , (1, -1))[0]]
                        updated_unlabeled_data = TorchDataset(
                            self.unlabeled[updated_indices - len(self.train_dataset)]
                        )
                        # TODO: 将updated data 和 training data合并
                        self.concat_dataset = ConcatTorchDataset([self.concat_dataset, updated_unlabeled_data])
                ...
                logits = outputs[0]
                y_hat = torch.log(softmax(outputs_pseudo[0]))
                max_class = torch.max(y_hat, 1).indices

                indices = torch.cat((indices, batch_ids), 0)
                batch_output = torch.cat((batch_output, max_class), 0)
                batch_label = torch.cat((batch_label, batch[3]), 0)
                # TEST1: take max value of convinces
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.logging_steps > 0 and global_step % self.args.self_train_logging_steps == 0:
                        self.evaluate('dev', global_step, 'student')
                        self.evaluate('test', global_step, 'student')
    ...
    def train(self):
        # Prepare optimizer and schedule (linear warmup and decay)
        # _, t_total = self.set_dataset(self.train_dataset, self.unlabeled)

        train_dataloader, t_total = self.set_dataset()
        no_decay = ['bias', 'LayerNorm.weight']
        optimizer_grouped_parameters = [
            {'params': [p for n, p in self.model.named_parameters() if not any(nd in n for nd in no_decay)],
             'weight_decay': self.args.weight_decay},
        ...
        softmax = nn.Softmax(dim=1)
        high_confidence_class_list = []
        for _ in train_iterator:
            # train_dataloader, t_total = self.set_dataset(self.train_dataset, self.unlabeled)
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                # high_confidence_class_list.append((torch.max(y_hat, dim=1, keepdim=True)[1], y_hat.shape))
                # high_confidence_class_list.append((y_hat, y_hat.shape))

                loss = criterion(input=torch.log(softmax(logits)), target=self.label_matrix[batch[3]].to(self.device))
                if self.args.gradient_accumulation_steps > 1:
                    loss = loss / self.args.gradient_accumulation_steps
                if torch.cuda.device_count() > 1:
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    epoch_iterator.set_description(
                        "iteration:%d, w=%.1f, Loss:%.3f" % (_, self.args.soft_label_weight, tr_loss / global_step))
            ...
            # print(torch.reshape(torch.cat([x[0] for x in high_confidence_class_list], 1), (-1, )))
            if 0 < self.args.max_steps < global_step:
    ...
    def evaluate(self, mode, global_step=-1, model='teacher'):
        ...
        if self.args.task_type == 're':
            write_prediction_re(self.args, os.path.join(self.args.eval_dir, "pred", os.sep, "proposed_answers.txt"),
                                preds)
        elif self.args.task_type == 'tc':
            write_prediction_tc(self.args, os.path.join(self.args.eval_dir, "pred", os.sep, "pred_%s_%s_%s_%d.txt" % (
                self.args.task, mode, self.args.method, global_step)), preds, self.id2label)
        elif self.args.task_type == 'wic':
            write_prediction_wic(self.args, os.path.join(self.args.eval_dir, "pred", os.sep, "pred_%s_%s_%s_%s.txt" % (
                self.args.task, mode, self.args.method, str(global_step))), preds, self.id2label)
        else:
            pass
        ...
        write_f1_tc(self.args, os.path.join(self.args.eval_dir, "pred_%s_%s" % (self.args.task, mode)),
                    result["macro-f1"], result["micro-f1"], result["acc"], global_step)
    ...
    def save_model(self):
        ...
        logger.info("Saving teacher model checkpoint to %s", output_dir)
