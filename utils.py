    ...
    with open(output_file + '_%d.txt' % (args.rule), 'a+', encoding='utf-8') as f:
        ...
        if args.method == 'mt':
            ...
            if args.self_training_addvat and args.self_training_addmt:
                pred = '%s vat_mt e:%.1e_b:%.1e eps:%.2f reg:%.2f g:%.1f cyc:%d ' % (
                time.ctime(), args.vat_eps, args.mt_beta, args.bond_eps, args.self_training_reg,
                args.self_training_graph_loss, args.self_training_update_period) + basic_str
            elif args.self_training_addvat:
                assert args.self_training_addmt == 0
                pred = '%s vat_e:%.1e eps:%.2f reg:%.2f g:%.1f cyc:%d ' % (
                time.ctime(), args.vat_eps, args.bond_eps, args.self_training_reg, args.self_training_graph_loss,
                args.self_training_update_period) + basic_str
            elif args.self_training_addmt:
                assert args.self_training_addvat == 0
                pred = '%s mt_b:%.1e eps:%.2f reg:%.2f g:%.1f cyc:%d ' % (
                time.ctime(), args.mt_beta, args.bond_eps, args.self_training_reg, args.self_training_graph_loss,
                args.self_training_update_period) + basic_str
            else:
                assert args.self_training_addmt == 0 and args.self_training_addmt == 0
                pred = '%s eps:%.2f reg:%.2f g:%.1f cyc:%d ' % (
                time.ctime(), args.bond_eps, args.self_training_reg, args.self_training_graph_loss,
                args.self_training_update_period) + basic_str
            ...
            if args.self_training_addvat and args.self_training_addmt:
                ...
                assert args.self_training_addmt == 0 and args.self_training_addmt == 0
                pred = '%s eps:%.2f reg:%.2f g:%.1f cyc:%d maxratio:%.1f' % (
                time.ctime(), args.bond_eps, args.self_training_reg, args.self_training_graph_loss,
                args.self_training_update_period, args.curr_max_ratio) + basic_str

        elif args.method == 'sat':
            pred = '%s es:%d mom:%.1f a:%.1f b:%.1f ' % (
            time.ctime(), args.sat_es, args.sat_momentum, args.sat_alpha, args.sat_beta) + basic_str
        else:
