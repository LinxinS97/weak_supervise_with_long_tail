...
def tokenize_with_2span(tokenizer, sent, span_a, span_b):
    ...
    if span_a[1] <= span_b[0]:
        ...
        return _s0 + _wb + _s1 + _wa + _s2, \
               len(_s0) + len(_wb) + len(_s1), len(_s0) + len(_wb) + len(_s1) + len(_wa), len(_s0), len(_s0) + len(_wb)
...
def convert_examples_to_features_re(examples, max_seq_len, tokenizer,
    ...
    for (ex_index, example) in enumerate(examples[:]):
        ...
        tokens_a, key_a_start, key_a_end, key_b_start, key_b_end = tokenize_with_2span(tokenizer, example.text_a,
                                                                                       example.span_a, example.span_b)
        keys = [0] * len(tokens_a)
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    if os.path.exists(cached_features_file) and args.auto_load:
        ...
        if args.task_type == 'wic':
            features, = convert_examples_to_features_wic(examples, args.max_seq_len, tokenizer,
                                                         add_sep_token=args.add_sep_token, task=args.task_type)
        elif args.task_type == 're':
            features = convert_examples_to_features_re(examples, args.max_seq_len, tokenizer,
                                                       add_sep_token=args.add_sep_token, task=args.task_type)
        else:
    ...
    if args.task_type == 're':
        ...
        dataset = TorchDataset((all_input_ids, all_attention_mask,
                                all_token_type_ids, all_label_ids, all_e1_mask, all_e2_mask, all_ids))
